#!/usr/bin/env bash
#
# dev-up.sh - start the local development stack INSIDE the dev container.
#
#   dev-up.sh            start backend + frontend (background)
...
#
# The Azure emulators (Cosmos, Azurite) are Docker services managed by
# compose; they must be up already (see dev-stack.sh up). This script only
# manages the two application processes.
#
# Backend runs on :7239 to match frontend/public/config.js
# (apiUrl http://localhost:7239/api). Frontend runs on :5173.
#
set -euo pipefail
...
RUN_BACKEND=0 RUN_FRONTEND=0 RUN_DOWN=0 RUN_STATUS=0 RUN_LOGS=0
...
pick() {
  for arg in "$@"; do
    case "$arg" in
      ...
      --logs)     RUN_BACKEND=1 RUN_FRONTEND=1 RUN_LOGS=1 ;;
      --down)     RUN_DOWN=1 ;;
      --status)   RUN_STATUS=1 ;;
...
pick "$@"
[ "$#" -eq 0 ] && RUN_BACKEND=1 RUN_FRONTEND=1
...
if [ "$RUN_STATUS" -eq 1 ]; then
  echo "== ar-loans-app dev stack status =="
...
if [ "$RUN_DOWN" -eq 1 ]; then
  echo "== stopping ar-loans-app dev stack =="
...
# ---- backend (.NET Azure Functions, :7239) ------------------------------------
if [ "$RUN_BACKEND" -eq 1 ]; then
  echo "== starting backend (:7239) =="
  ( cd backend/Ar.Loans.Api && func start --port 7239 --address 0.0.0.0 ) >"$LOGDIR/backend.log" 2>&1 &
  echo $! > "$PIDFILE_BACKEND"
  wait_port 7239 backend || true
fi
...
if [ "$RUN_LOGS" -eq 1 ]; then
  ...
  echo "  backend   : http://localhost:7239   api base: /api"
  echo "  frontend  : http://localhost:5173 (https://localhost:5173 - basicSsl)"
